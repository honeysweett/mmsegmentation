...
val_dataloader = dict(
    dataset=dict(
        metainfo=metainfo,
        data_root=data_root,
        ann_file='mtsd_coco_format.json',
        data_prefix=dict(img='images/')))

test_dataloader = val_dataloader

val_evaluator = dict(ann_file=data_root + 'mtsd_coco_format.json')
...
#         custom_keys={
        ...
        # }
# 这种配置通常用于模型微调（fine-tuning），其中一些参数（如预训练的backbone或语言模型）可能被冻结，不参与训练，
# 或者使用不同的学习率和权重衰减策略。
optim_wrapper = dict( 
